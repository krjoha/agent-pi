...
// ABOUTME: Validates mode classification guidance, Commander integration, and chain status reporting.
...
describe("buildNormalPrompt", () => {
	it("is a non-empty string", () => {
		const result = buildNormalPrompt({ commanderAvailable: false, activeChain: null });
	...
	it("contains all 5 mode names", () => {
		const result = buildNormalPrompt({ commanderAvailable: false, activeChain: null });
		for (const mode of ["NORMAL", "PLAN", "SPEC", "TEAM", "CHAIN"]) {
			expect(result).toContain(mode);
		}
	});

	it("with commanderAvailable: true, contains commander_task", () => {
		const result = buildNormalPrompt({ commanderAvailable: true, activeChain: null });
	...
	it("with activeChain set, contains chain name", () => {
		const result = buildNormalPrompt({ commanderAvailable: false, activeChain: "plan-build-review" });
...
describe("buildNormalPrompt — Scout delegation", () => {
	...
	it("with scoutId: null, does not contain scout instructions", () => {
		const result = buildNormalPrompt({ commanderAvailable: false, activeChain: null, scoutId: null });
		expect(result).not.toContain("Scout Agent");
		expect(result).not.toContain("subagent_continue");
	});

	it("with scoutId set, contains scout delegation section", () => {
		const result = buildNormalPrompt({ commanderAvailable: false, activeChain: null, scoutId: 1 });
	...
	it("with scoutId set, references the correct SA ID", () => {
		const result = buildNormalPrompt({ commanderAvailable: false, activeChain: null, scoutId: 42 });
